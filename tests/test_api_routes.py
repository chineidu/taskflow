...
@pytest.mark.asyncio
async def test_submit_job(client: TestClient, db_session: AsyncSession) -> None:  # noqa: ARG001
    ...
    with patch("src.api.routes.v1.jobs.atrigger_job", new_callable=AsyncMock) as mock_trigger:
        ...
        # We need to commit the session in the app, which happens automatically in the route
        # But we need to check the DB. Since client and test share the session (via override),
        # we should be able to query it.
        # However, the route uses a separate session from `aget_db` dependency.
        # In conftest.py, we overrode `aget_db` to yield `db_session`.
        # So they share the same session object.

        # We might need to expire_all or refresh to see changes if the session is the same instance
        # but used in different contexts.
        # Actually, `db_session` fixture yields a session.
        # The app uses that same session.
        # The app commits.
        # We can query using that session.
...
@pytest.mark.asyncio
async def test_get_job_status(client: TestClient, db_session: AsyncSession) -> None:
...
@pytest.mark.asyncio
async def test_get_job_not_found(client: TestClient) -> None:
...
@pytest.mark.asyncio
async def test_get_task_logs(client: TestClient, db_session: AsyncSession) -> None:
...
@pytest.mark.asyncio
async def test_get_task_logs_no_logs(client: TestClient, db_session: AsyncSession) -> None:
