...
SLEEP_SECONDS = 600  # Sleep duration to allow logger to flush
...
async def arun_cleanup() -> None:
    ...
    if cleaned_rows:
        logger.info(f"[DONE] Cleaned up {cleaned_rows} expired tasks.")
    else:
        logger.info("[DONE] No expired tasks to clean up.")
